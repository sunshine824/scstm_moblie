        ...
        <div class="order-list" v-if="reserList.status===0 && reserList.data.data.length">
          <div class="order-item" :key="index" v-for="(item,index) in reserList.data.data">
            <div class="info">
              ...
              <p class="status" v-if="item.status!==1">
                {{statusObj[item.status]}}
              </p>
              <p class="status" v-if="item.status===1">
                {{item.overdue_time | countTime}}
              </p>
            </div>
            <div class="act-info">
              <div class="act-img">
            ...
            <p class="money"><span style="color: #ff8000;">{{ticketObj[item.drawer]}}</span> &nbsp;&nbsp;
              共{{item.name_user.split(',').length}}位预约人 &nbsp;&nbsp; 合计：<span>{{item.money}}</span></p>
            <div class="btns">
              <Button v-if="item.status===1 || item.status===2" @click="reserCancel(item)" class="btn ivu-btn-warning">
                取消预约
              </Button>
              ...
              <Button class="btn" v-if="item.status===2 && item.drawer===3" @click="renderQrcode(item)">生成订单二维码</Button>
            </div>
          </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        ticketObj: {1: '未出票', 2: '出票中', 3: '出票成功', 4: '出票失败', 5: '已退票'},
        navBar: [
          {title: '未付款', id: 1},
          {title: '已付款', id: 2},
          {title: '全部订单', id: ''},
      ...
      otherPay(item, id) {
        ...
        result.then(res => {
          ...
          if (res.status === 0) {
            ...
            this.showDialog({type: '', title: '温馨提示', content: '调用支付失败，请稍后重试！', showClose: false})
          }
        }).catch(err => {
          this.loading = false
          console.log(err)
      ...
      reserCancel(item) {
        ...
        result.then(res => {
          ...
          if (res.status === 0) {
            ...
            this.showDialog({type: '', title: '温馨提示', content: '取消预约失败，请稍后重试！', showClose: false})
          }
        }, (err) => {
    ...
    filters: {
      filterLink([type, id]) {
        return (type == '教育活动' ? '/edu/' : type == '美科新' ? '/mkx/' : type == '4D影院' ? '/theater/' : '') + id
      },
      formatDate(date) {
